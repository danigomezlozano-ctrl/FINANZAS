...
def _nasdaq_universe():
    """Cribado masivo gratuito del universo de acciones USA (Nasdaq screener).
    Nasdaq bloquea peticiones sin cabeceras de navegador completas."""
    ...
    data, err = _get(url, headers=headers)
    if err or not data:
        return [], f"Nasdaq screener: {err or 'sin datos'}"
    rows = data.get("data", {}).get("rows", [])
    if not rows:
        return [], "Nasdaq screener: respuesta vacía"
    return rows, None
...
def radar_convex_stocks():
    """
    Radar de descubrimiento de acciones (la 'próxima NVDA'), 100% gratis.
    Embudo de 3 filtros para encontrar empresas jóvenes de sectores emergentes
    ANTES de que sean obvias, con fundamentales reales (no momentum de precio):
      F1: universo Nasdaq (~7000 empresas, gratis sin key)
      F2: mid-cap 2-30B + sector growth + IPO reciente (2018+)
      F3: fundamentales FMP de los finalistas -> crecimiento + márgenes
    Solo sube al top quien tiene tesis tipo-NVDA: crecimiento sano y márgenes.
    """
    # F1: cribado masivo gratuito
    universe, err = _nasdaq_universe()
    if err:
        return [], f"radar acciones: {err}"

    # F2: filtro estructural (gratis, sin API de pago)
    GROWTH_SECTORS = {"Technology", "Health Care", "Energy"}
    finalists = []
    for r in universe:
        mcap = _pf(r.get("marketCap", "0"))
        if not (2e9 <= mcap <= 30e9):       continue   # ni gigante ni casino
        if r.get("sector", "") not in GROWTH_SECTORS: continue
        try:
            ipo_year = int(r.get("ipoyear") or 0)
        except Exception:
            ipo_year = 0
        if ipo_year and ipo_year < 2018:    continue   # priorizar disrupción joven
        finalists.append({
            "symbol": r.get("symbol", ""),
    ...
    finalists.sort(key=lambda x: x["mcap_b"], reverse=True)
    finalists = finalists[:30]   # límite para no agotar FMP

    if not finalists:
        return [], "radar acciones: sin finalistas tras filtro estructural"

    # F3: fundamentales reales de los finalistas (FMP gratis)
    scored = []
    fmp_errors = 0
    for c in finalists:
        q, ferr = fmp_fundamentals(c["symbol"])
        if ferr or not q:
            fmp_errors += 1
            time.sleep(0.2)
            continue
        thesis = analyze_fundamentals(q)
        if thesis.get("status") in ("VERDE", "AMBAR") and thesis.get("rev_growth_latest") is not None:
            c["rev_growth"] = thesis["rev_growth_latest"]
            c["gross_margin"] = thesis.get("gross_margin")
            c["thesis"] = thesis["status"]
            # Puntuación: prioriza crecimiento alto + margen sano (tesis tipo-NVDA)
            growth = thesis["rev_growth_latest"]
            margin = thesis.get("gross_margin") or 0
            c["score"] = round(growth * 1.0 + margin * 0.3, 1)
            scored.append(c)
        time.sleep(0.2)

    # Si FMP falló en todos, avisar (autovigilancia)
    if not scored and fmp_errors >= len(finalists):
        return [], f"radar acciones: FMP no respondió ({fmp_errors} fallos)"

    scored.sort(key=lambda x: x.get("score", 0), reverse=True)
    return scored[:6], None
...
def build_report(thesis, radar_crypto, radar_stocks, is_weekly):
    ...
    # Radar de descubrimiento — acciones jóvenes con tesis tipo-NVDA
    if radar_stocks:
        lines.append("<b>RADAR DESCUBRIMIENTO (posibles futuras NVDA)</b>")
        for c in radar_stocks[:5]:
            g = c.get("rev_growth")
            m = c.get("gross_margin")
            extra = f" · crec {g:+.0f}%" if g is not None else ""
            extra += f" · margen {m:.0f}%" if m is not None else ""
            lines.append(f"• {c['symbol']} ({c['name']}) {c['sector']}{extra}")
        lines.append("<i>Empresas jóvenes de sectores emergentes con fundamentales sanos. Para estudiar, no comprar a ciegas.</i>")
        lines.append("")
...
def main():
    ...
    # CAPA 2
    print("-> Radar de candidatos...")
    radar_crypto, rc_err = radar_convex_crypto()
    radar_stocks, rs_err = radar_convex_stocks()
    radar_failures = [e for e in (rc_err, rs_err) if e]
    print(f"   Cripto: {len(radar_crypto)} candidatos | Acciones: {len(radar_stocks)} candidatos")
    ...
    if is_weekly or has_red:
        report = build_report(thesis, radar_crypto, radar_stocks, is_weekly)
