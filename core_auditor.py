...
# Confirmación de deterioro a LARGO PLAZO (cartera de largo plazo).
# La tesis debe estar rota de forma sostenida antes de recomendar transición.
THESIS_BREAK_READINGS = 6   # nº de lecturas rojas consecutivas (~6 meses si corre semanal el informe)
THESIS_STATE_FILE = "core_thesis_state.json"
...
# ───────────────────────────────────────────────────────────────────────
# MEMORIA TEMPORAL DE TESIS + MÓDULO DE TRANSICIÓN DULCE
# Una recomendación de transición solo se dispara cuando la tesis de un
# activo del core lleva ROTA de forma sostenida (largo plazo), Y existe
# una alternativa CONFIRMADA y sana para su mismo rol. Si no hay
# alternativa probada, el sistema lo admite con honestidad.
# ───────────────────────────────────────────────────────────────────────
def load_thesis_state():
    try:
        with open(THESIS_STATE_FILE) as f:
            return json.load(f)
    except Exception:
        return {}

def save_thesis_state(state):
    try:
        with open(THESIS_STATE_FILE, "w") as f:
            json.dump(state, f, indent=2)
    except Exception as e:
        print(f"[thesis_state] no se pudo guardar: {e}")

def update_thesis_memory(thesis_results):
    """
    Registra rachas de deterioro por activo. Devuelve dict:
      {asset: {"red_streak": n, "confirmed_broken": bool}}
    confirmed_broken = la tesis lleva rota >= THESIS_BREAK_READINGS lecturas.
    """
    state = load_thesis_state()
    out = {}
    for aid in ("NVDA", "SEMI", "BTC"):
        r = thesis_results.get(aid, {})
        st = r.get("status")
        prev = state.get(aid, {"red_streak": 0, "history": []})
        if st == "ROJO":
            prev["red_streak"] = prev.get("red_streak", 0) + 1
        elif st in ("VERDE", "AMBAR"):
            prev["red_streak"] = 0   # se recupera -> reinicia (era ruido)
        # AMBAR no rompe la racha de recuperación pero tampoco suma roja
        prev["history"] = (prev.get("history", []) + [{"date": DATE_ES[:10], "status": st}])[-12:]
        prev["confirmed_broken"] = prev["red_streak"] >= THESIS_BREAK_READINGS
        state[aid] = prev
        out[aid] = {"red_streak": prev["red_streak"],
                    "confirmed_broken": prev["confirmed_broken"]}
    save_thesis_state(state)
    return out

def find_role_alternative(role, watchlist):
    """
    Busca una alternativa CONFIRMADA y sana para un rol dado.
    Solo devuelve algo si el radar tiene candidatas que pasaron el año de prueba.
    role: 'convex' (BTC) o 'growth' (NVDA/SEMI).
    """
    confirmed = watchlist.get("confirmed", [])
    if not confirmed:
        return None
    # De las confirmadas, las que encajan en el rol (sector)
    role_sectors = {
        "convex": {"Technology", "Communication Services", "Energy"},
        "growth": {"Technology", "Healthcare", "Communication Services"},
    }.get(role, set())
    matches = [c for c in confirmed if c.get("sector") in role_sectors]
    return matches[0] if matches else None

def build_transitions(thesis_results, thesis_memory, watchlist):
    """
    Genera recomendaciones de transición SOLO si:
      1. La tesis está rota de forma confirmada (largo plazo).
      2. Hay (o no) alternativa confirmada -> lo refleja con honestidad.
    Devuelve lista de dicts de transición.
    """
    transitions = []
    for aid in ("NVDA", "SEMI", "BTC"):
        mem = thesis_memory.get(aid, {})
        if not mem.get("confirmed_broken"):
            continue   # aún no es deterioro sostenido -> no se recomienda nada
        role = CORE_ASSETS[aid].get("role", "")
        role_key = "convex" if role == "convex" else "growth"
        alt = find_role_alternative(role_key, watchlist)
        transitions.append({
            "asset": aid,
            "name": CORE_ASSETS[aid]["name"],
            "role": role,
            "red_streak": mem.get("red_streak"),
            "alternative": alt,   # puede ser None
        })
    return transitions
...
def build_report(thesis, radar_crypto, discovery, watchlist, transitions, is_weekly):
    ...
    # TRANSICIONES — solo se anuncian si el deterioro está CONFIRMADO a largo plazo
    if transitions:
        lines.append("<b>⚠️ TRANSICIÓN RECOMENDADA</b> (tesis rota sostenida)")
        for t in transitions:
            lines.append(f"🔴 <b>{t['name']}</b>: la tesis lleva rota {t['red_streak']} lecturas seguidas. Ya no cumple su rol ({t['role']}).")
            alt = t["alternative"]
            if alt:
                lines.append(f"   → Alternativa confirmada para su rol: <b>{alt['symbol']}</b> ({alt['name']}), tesis probada ≥1 año.")
            else:
                lines.append(f"   → Sin alternativa probada todavía para su rol. Opciones: mantener vigilando o salir a liquidez. NO hay un sustituto demostrado.")
            # Aviso de pérdidas si aplica (honestidad sobre vender en rojo)
            if t["asset"] == "BTC":
                lines.append(f"   ⚠️ OJO: si BTC está en pérdidas, vender supone asumirlas. Decisión tuya — el sistema no te empuja a vender en rojo.")
        lines.append("")

    # Vigilancia temprana (informativo, NO recomendación) — tesis en deterioro aún no confirmado
    watching = [aid for aid in ["NVDA","SEMI","BTC"]
                if thesis.get(aid,{}).get("status") in ("AMBAR","ROJO")
                and not any(t["asset"]==aid for t in transitions)]
    if watching:
        lines.append("<b>👁 VIGILANCIA</b> (deterioro no confirmado, solo seguimiento)")
        for aid in watching:
            r = thesis[aid]
            emj = "🔴" if r["status"]=="ROJO" else "🟡"
            lines.append(f"{emj} {CORE_ASSETS[aid]['name']}: {r.get('reason','')[:90]}")
        lines.append("<i>Una sola lectura no basta. El sistema espera confirmación sostenida antes de recomendar nada.</i>")
        lines.append("")
...
def main():
    ...
    # MEMORIA TEMPORAL DE TESIS — confirma deterioro a largo plazo
    print("-> Actualizando memoria de tesis (confirmación largo plazo)...")
    thesis_memory = update_thesis_memory(thesis)
    for aid, m in thesis_memory.items():
        if m["red_streak"] > 0:
            estado = "CONFIRMADO ROTO" if m["confirmed_broken"] else f"racha roja {m['red_streak']}/{THESIS_BREAK_READINGS}"
            print(f"   {aid}: {estado}")
    ...
    # MÓDULO DE TRANSICIÓN — solo si deterioro confirmado + busca alternativa
    transitions = build_transitions(thesis, thesis_memory, watchlist)
    for t in transitions:
        alt = t["alternative"]
        if alt:
            print(f"   ⚠️ TRANSICIÓN {t['name']}: tesis rota sostenida. Alternativa: {alt['symbol']}")
        else:
            print(f"   ⚠️ TRANSICIÓN {t['name']}: tesis rota sostenida, SIN alternativa probada aún")
    ...
    if is_weekly or has_transition:
        report = build_report(thesis, radar_crypto, discovery, watchlist, transitions, is_weekly)
