...
def radar_convex_stocks():
    """
    Busca acciones de alto crecimiento (rol convexo) vía FMP screener.
    Filtro: volatilidad alta (beta) + tamaño medio + sectores de crecimiento.
    El plan gratuito de FMP puede limitar el screener; si pasa, se detecta.
    """
    if not FMP_KEY:
        return [], "FMP key no configurada"
    # Sectores de crecimiento donde suelen aparecer las "próximas NVDA"
    ...
    for sector in growth_sectors:
        url = (f"https://financialmodelingprep.com/stable/company-screener"
               f"?marketCapMoreThan=1000000000&marketCapLowerThan=50000000000"
               f"&betaMoreThan=1.5&sector={urllib.parse.quote(sector)}"
               f"&isActivelyTrading=true&limit=10&apikey={FMP_KEY}")
        data, err = _get(url)
        if err:
            last_err = err
            continue
        if isinstance(data, list):
            for s in data:
                mcap = s.get("marketCap", 0) or 0
                out.append({
                    "symbol": s.get("symbol", ""),
                    "name": (s.get("companyName", "") or "")[:30],
                    "sector": s.get("sector", "") or sector,
                    "mcap_b": round(mcap/1e9, 1),
                    "beta": s.get("beta"),
                })
        time.sleep(0.3)  # respetar rate limit
    # Ordenar por beta (volatilidad = capacidad de gran movimiento)
    out = [s for s in out if s["symbol"]]
    out.sort(key=lambda x: x.get("beta") or 0, reverse=True)
    # Deduplicar por símbolo
    seen = set(); dedup = []
    for s in out:
        if s["symbol"] not in seen:
            seen.add(s["symbol"]); dedup.append(s)
    if not dedup and last_err:
        return [], f"radar acciones: screener no disponible en plan free ({last_err})"
    if not dedup:
        return [], "radar acciones: sin resultados (screener puede estar limitado en plan free)"
    return dedup[:8], None
...
def build_report(thesis, radar_crypto, radar_stocks, is_weekly):
    ...
    # Radar convexo — solo top candidatos limpios
    if radar_crypto:
        top = ", ".join(f"{c['symbol']}" for c in radar_crypto[:5])
        lines.append(f"<b>RADAR CONVEXO (cripto)</b>: {top}")
        lines.append("<i>Universo filtrado (sin stablecoins ni micro-caps). No es recomendación de compra.</i>")
        lines.append("")
